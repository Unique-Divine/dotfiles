...
"#,
    ...
    #[test]
    fn zero_indexed_tab_shortcut_maps_to_matching_navigation_action() {
        let mut state = state_with_workspaces(&["test"]);
        let config: Config = toml::from_str("[keys]\nswitch_tab = \"prefix+0..8\"\n").unwrap();
        state.keybinds.switch_tab = config.keybinds().switch_tab;

        let first = action_for_key(
            &state,
            TerminalKey::new(KeyCode::Char('0'), KeyModifiers::empty()),
            BindingDispatch::Prefix,
        );
        let fourth = action_for_key(
            &state,
            TerminalKey::new(KeyCode::Char('3'), KeyModifiers::empty()),
            BindingDispatch::Prefix,
        );

        assert_eq!(first, Some(NavigateAction::SwitchTab(0)));
        assert_eq!(fourth, Some(NavigateAction::SwitchTab(3)));
    }
