            ...
            match parse_binding_string(raw) {
                ...
                Some(ParsedBinding::Range(_, range)) => {
                    labels.extend(range.into_iter().filter_map(|binding| {
                        matches!(binding.trigger.combo().0, KeyCode::Char('0'..='9'))
...
impl Keybinds {
    /// Returns the display base implied by the configured tab shortcuts.
    ///
    /// A mixed set of zero- and one-indexed bindings keeps the established
    /// one-indexed presentation because it has no unambiguous display mode.
    pub fn tab_display_index_base(&self) -> usize {
        (!self.switch_tab.is_empty()
            && self
                .switch_tab
                .iter()
                .all(|binding| binding.index_base == 0))
        .then_some(0)
        .unwrap_or(1)
    }
}
...
enum ParsedBinding {
    ...
    Range(IndexedRange, Vec<ResolvedBinding>),
}
...
fn parse_indexed_bindings(
    ...
    for raw in config.values() {
        ...
        match parse_binding_string(raw) {
            ...
            Some(ParsedBinding::Range(range_kind, range)) => {
                if range_kind == IndexedRange::ZeroBased && field != "keys.switch_tab" {
                    let diag = format!(
                        "zero-indexed range is only valid for keys.switch_tab: {field} = {raw:?}; disabling binding"
                    );
                    warn!(message = %diag, "config diagnostic");
                    diagnostics.push(diag);
                    continue;
                ...
                for binding in range {
                    push_indexed_binding(
                        field,
                        binding,
                        range_kind == IndexedRange::ZeroBased,
                        registry,
                        diagnostics,
                        source,
                        &mut bindings,
...
fn push_indexed_binding(
    ...
    let valid_key = if zero_indexed {
        matches!(binding.trigger.combo().0, KeyCode::Char('0'..='8'))
    } else {
        matches!(binding.trigger.combo().0, KeyCode::Char('1'..='9'))
    };
    if !valid_key {
        let valid_range = if zero_indexed { "0..8" } else { "1..9" };
        let diag = format!(
    ...
    registry.register(&binding, field, source);
    let KeyCode::Char(key_number) = binding.trigger.combo().0 else {
        unreachable!("validated indexed keybinding must use a character key");
    };
    let index = if zero_indexed {
        (key_number as usize) - ('0' as usize)
    } else {
        (key_number as usize) - ('1' as usize)
    };
    bindings.push(IndexedKeybind {
        trigger: binding.trigger,
        label: binding.label,
        index,
        index_base: usize::from(!zero_indexed),
    });
}
...
fn parse_binding_string(raw: &str) -> Option<ParsedBinding> {
    ...
    if let Some((range_modifiers, range)) = parse_range_modifiers(body) {
        let digits = match range {
            IndexedRange::OneBased => 1..=9,
            IndexedRange::ZeroBased => 0..=8,
        };
        let bindings = digits
            .map(|idx| {
                let combo = (
            ...
            .collect();
        return Some(ParsedBinding::Range(range, bindings));
    }
...
fn parse_range_modifiers(s: &str) -> Option<(KeyModifiers, IndexedRange)> {
    let mut modifiers = KeyModifiers::empty();
    let mut range = None;
    for part in s.split('+') {
        let trimmed = part.trim();
        let parsed_range = match trimmed {
            "1..9" => Some(IndexedRange::OneBased),
            "0..8" => Some(IndexedRange::ZeroBased),
            _ => None,
        };
    ...
    range.map(|range| (modifiers, range))
}
...
"#,
    ...
    #[test]
    fn zero_indexed_tab_range_maps_digits_to_matching_tab_positions() {
        let config: Config = toml::from_str(
            r#"
[keys]
...
"#,
        ...
        assert_eq!(keybinds.switch_tab.len(), 9);
        assert_eq!(keybinds.tab_display_index_base(), 0);
        assert_eq!(keybinds.switch_tab[0].label, "prefix+0");
        assert_eq!(
            keybinds.switch_tab[0].matched_index(&TerminalKey::new(
                KeyCode::Char('0'),
                KeyModifiers::empty(),
            )),
            Some(0)
        ...
        assert_eq!(
            keybinds.switch_tab[8].matched_index(&TerminalKey::new(
                KeyCode::Char('8'),
                KeyModifiers::empty(),
            )),
            Some(8)
    ...
    #[test]
    fn zero_indexed_range_is_rejected_for_non_tab_actions() {
        let config: Config = toml::from_str(
            r#"
[keys]
switch_workspace = "prefix+0..8"
"#,
        )
        .unwrap();
        let diagnostics = config.collect_diagnostics();

        assert!(config.keybinds().switch_workspace.is_empty());
        assert!(diagnostics
            .iter()
            .any(|diag| diag.contains("only valid for keys.switch_tab")));
    }

    #[test]
    fn mixed_tab_ranges_keep_one_indexed_display_labels() {
        let config: Config = toml::from_str(
            r#"
[keys]
...
"#,
        ...
        assert_eq!(config.keybinds().tab_display_index_base(), 1);
    }
