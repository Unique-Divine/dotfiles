...
fn indexed_label(bindings: &[crate::config::IndexedKeybind]) -> String {
    ...
    while index < bindings.len() {
        if let Some((prefix, first_digit)) = indexed_range_prefix(&bindings[index..]) {
            let last_digit = char::from(first_digit as u8 + 8);
            parts.push(format!("{prefix}{first_digit}..{last_digit}"));
            index += 9;
        } else {
...
fn indexed_range_prefix(bindings: &[crate::config::IndexedKeybind]) -> Option<(&str, char)> {
    let run = bindings.get(..9)?;
    let first_digit = run[0]
        .label
        .chars()
        .last()
        .filter(|digit| matches!(digit, '0' | '1'))?;
    let prefix = run[0].label.strip_suffix(first_digit)?;
    for (offset, binding) in run.iter().enumerate() {
        let digit = char::from(first_digit as u8 + offset as u8);
        if binding.label.strip_suffix(digit) != Some(prefix) {
            return None;
        }
    }
    Some((prefix, first_digit))
}
...
pub(super) fn keybind_help_groups(app: &AppState) -> Vec<HelpGroup> {
    ...
    let workspace_tab = vec![
        ...
        help_entry(indexed_label(&kb.switch_tab), "switch tab"),
...
mod tests {
    ...
    #[test]
    fn indexed_label_preserves_zero_indexed_tab_range() {
        let config: crate::config::Config =
            toml::from_str("[keys]\nswitch_tab = \"prefix+0..8\"\n").unwrap();

        assert_eq!(indexed_label(&config.keybinds().switch_tab), "prefix+0..8");
    }
}
