...
fn render_header_status(
    ...
    let tab_label = mobile_tab_status(ws, app.keybinds.tab_display_index_base());
...
fn mobile_tab_status(ws: &crate::workspace::Workspace, index_base: usize) -> String {
    let tab_label = ws
        .tab_display_name_with_index_base(ws.active_tab, index_base)
        .unwrap_or_else(|| (ws.active_tab + index_base).to_string());
    if ws.tabs.len() <= 1 {
...
fn render_mobile_switcher_content(
        ...
        space_entries.iter().enumerate()
    {
        ...
        let detail = format!(
            ...
            mobile_tab_status(ws, app.keybinds.tab_display_index_base())
        );
        render_two_line_item(
            frame,
            viewport,
    ...
    if let Some(ws) = app.active.and_then(|idx| app.workspaces.get(idx)) {
        ...
        for (idx, tab) in ws.tabs.iter().enumerate() {
            ...
            let bg = mobile_item_bg(false, active, p);
            let display_name = ws
                .tab_display_name_with_index_base(idx, app.keybinds.tab_display_index_base())
                .unwrap_or_else(|| (idx + app.keybinds.tab_display_index_base()).to_string());
            let label = if tab.is_auto_named() {
                format!("tab {display_name}")
            } else {
                format!(
                    "{} · {display_name}",
                    idx + app.keybinds.tab_display_index_base()
                )
            };
            let title = Line::from(vec![
                Span::styled("  ", Style::default().bg(bg)),
...
mod tests {
    ...
    fn mobile_tab_status_uses_compact_tab_label_and_position() {
        ...
        assert_eq!(mobile_tab_status(&workspace, 1), "tab 2 · 2/2");
    }

    #[test]
    fn mobile_tab_status_uses_zero_indexed_generated_label() {
        let mut workspace = crate::workspace::Workspace::test_new("mobile-tabs");
        workspace.test_add_tab(None);
        workspace.active_tab = 1;

        assert_eq!(mobile_tab_status(&workspace, 0), "tab 1 · 2/2");
    }
