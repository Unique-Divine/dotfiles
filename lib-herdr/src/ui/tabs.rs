...
fn tab_width(ws: &crate::workspace::Workspace, tab_idx: usize) -> u16 {
    display_width_u16(&tab_chrome_label(ws, tab_idx, 1))
        .saturating_add(4)
        .max(MIN_TAB_WIDTH)
}
...
fn tab_chrome_label(
    ...
    let name = ws
        .tab_display_name_with_index_base(tab_idx, index_base)
...
pub(super) fn render_tab_bar(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    for (idx, tab) in ws.tabs.iter().enumerate() {
        ...
        let name = tab_chrome_label(ws, idx, app.keybinds.tab_display_index_base());
